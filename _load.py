...
from desc.grid import Grid  # Import Grid
import numpy as np
# from desc.transform import Transform
from tqdm import tqdm
# from a5py import Ascot as a5io
from a5py.ascot5io.bfield import B_STS as bsts
...
@parseunits(phimin='deg', phimax='deg')
def desc_field(h5file: str, phimin: float=0, phimax: float=360, 
               nphi=361, ntheta=120, nr=100, nz=100, psipad=0.000, 
               rescale_R=None, rescale_B=None):
    ...
    theta_2D = grid.nodes[:, 1].reshape((grid.num_zeta, grid.num_theta), order="C").T
    phi_2D = grid.nodes[:, 2].reshape((grid.num_zeta, grid.num_theta), order="C").T
    ...
    for k in tqdm(range(nphi), desc="Interpolating DESC field", total=nphi):
